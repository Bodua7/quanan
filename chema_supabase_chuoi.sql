...
-- clientId: id sinh ở máy khách ngay trước khi gửi đơn, dùng để app tự
-- kiểm tra lại "đơn này đã tới quán chưa?" khi mạng rớt đúng lúc chờ phản
-- hồi (đơn đã lưu vào DB nhưng khách lại thấy báo lỗi mất kết nối), và để
-- tránh tạo đơn trùng nếu khách bấm gửi lại. Chỉ ràng buộc unique khi có
-- giá trị (đơn cũ trước khi có cột này sẽ có clientId = null, không bị chặn).
create unique index if not exists orders_client_id_uniq on orders ("clientId") where "clientId" is not null;
